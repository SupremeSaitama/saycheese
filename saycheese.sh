...
link=$(curl -s -N http://127.0.0.1:4040/api/tunnels | grep -o "https://[0-9a-z]*\.ngrok.com")
...
cloudflared tunnel --url http://localhost:3333 > /dev/null 2>&1 &

sleep 5
url=$(cloudflared tunnel --url http://loacalhost:3333 | grep -o 'https://.*trycloudflare.com')
echo "[+] Your link: $url"
